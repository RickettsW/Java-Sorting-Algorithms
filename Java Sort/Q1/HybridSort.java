...
public class HybridSort {
    ...
    public static int[] selectionSort(int[] array) {
        /*argorithm for selection sort, used in hybrid sort */
        int n = array.length;
        for (int i = 0; i < n - 1; i++) {
            int minIndex = i;
            for (int j = i + 1; j < n; j++) {
    ...
    public static int[] sortHybridQuick(int[] array) {
        /* mix of selection sort and quicksort */
        int cutoff = 10; /* amount of items left before selection sort starts */
        if (array.length <= cutoff) {
            return selectionSort(array); /*basecase */
        }
